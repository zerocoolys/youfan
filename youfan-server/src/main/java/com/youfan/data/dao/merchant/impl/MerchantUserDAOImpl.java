...
public class MerchantUserDAOImpl implements MerchantUserDAO {
	...
	public MerchantUserVO saveMerchantUserInfo(MerchantUserVO merchantUser) {
		if (mongoTemplate.collectionExists(getEntityClass())) {
			...
			update.set("headPortraitPicUrl",
					merchantUser.getHeadPortraitPicUrl());
			update.set("healthCertificatePicUrl",
					merchantUser.getHealthCertificatePicUrl());
			...
			return convertToVO(mongoTemplate.findAndModify(query(where("id")
					.is(merchantUser.getId())), update, getEntityClass()));
		} else {
	...
	@Override
	public MerchantUserVO findOne(Long mui) {

		return mongoTemplate.findOne(query(where("mui").is(mui)),
				MerchantUserVO.class);
	...
	public MerchantUserVO login(String userName) {
		...
		if (!mongoTemplate.collectionExists(getEntityClass())) {
			...
			if (mongoTemplate.findOne(query(where("userName").is(userName)),
					getEntityClass()) == null) {
				mongoTemplate.insert(merchantUser);
			}
		}
		MerchantUserEntity merchantUserEntity = mongoTemplate.findOne(
				query(where("userName").is(userName)), getEntityClass());
	...
	public Map<String, String> register(String userName, String passWord) {
		...
		if (mongoTemplate.findOne(query(where("userName").is(userName)),
			...
			MerchantUserEntity merchantUserEntityRes = mongoTemplate.findOne(
					query(where("userName").is(userName).andOperator(
							where("passWord").is(passWord))), getEntityClass());
	...
	public List<MerchantUserEntity> getMerchantByStatus(Integer status) {
		return mongoTemplate.find(query(where("status").is(status)),
				getEntityClass());
	}
	...
	public void updateStatus(String id, Integer status) {
		mongoTemplate.updateFirst(query(where("id").is(id)),
				Update.update("status", status), getEntityClass());
	}
	...
	public MerchantUserVO findById(String id) {
		Query q = new Query();
		Criteria c = Criteria.where(Constants.DATA_ID).is(id).and("status")
				.is(0);
		q.addCriteria(c);
