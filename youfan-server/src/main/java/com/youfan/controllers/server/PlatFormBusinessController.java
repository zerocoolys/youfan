...
public class PlatFormBusinessController {
	...
	public Response saveCoupons(HttpServletRequest request,
			HttpServletResponse response) {
		Response res = null;
		try {
			if (request.getParameter("port") != null
					&& request.getParameter("timeLine") != null
				...
				coupons.setTimeLine(Integer.valueOf(request
						.getParameter("timeLine")));
				coupons.setKitchenId(request.getParameter("kitchenId"));
				coupons.setDesc(request.getParameter("desc"));
				coupons.setContent(JSONUtils.getObjectListByJson(
						request.getParameter("content"),
						CouponsContentEntity.class));
	...
	public Response checkWords(@PathVariable String sentence,
	...
	public Response getOrders(HttpServletRequest request,
			HttpServletResponse response) {
		...
		try {
			...
			if (request.getParameter("orderStatus") != null) {
				op.setOrderStatus(Integer.valueOf(request
			...
			if (ifPager) {
				CollectionVO<OrderVO> payload = new CollectionVO<>(
						new ArrayList<OrderVO>(), recordCnt,
	...
	public Response saveCouponsType(HttpServletRequest request,
			HttpServletResponse response) {
		Response res = null;
		try {
			if (request.getParameter("port") != null
	...
	public Response getCouponsType(HttpServletRequest request,
			HttpServletResponse response) {
		Response res = null;
		try {
			CouponsParams couponsParams = new CouponsParams();
			if (request.getParameter("port") != null) {
				couponsParams.setPort(Integer.valueOf(request
						.getParameter("port")));
			}
			if (request.getParameter("timeLine") != null) {
				couponsParams.setTimeLine(Integer.valueOf(request
						.getParameter("timeLine")));
			}
			...
			if (request.getParameter("status") != null) {
				couponsParams.setStatus(Integer.valueOf(request
						.getParameter("status")));
			}
			long recordCnt = couponsTypeService.count(couponsParams);
			if (request.getParameter("pageSize") != null
					&& request.getParameter("pageNo") != null) {
				couponsParams.setPageSize(Integer.valueOf(request
						.getParameter("pageSize")));
				couponsParams.setPageNo(Integer.valueOf(request
						.getParameter("pageNo")));
			} else {
				couponsParams.setPageSize((int) recordCnt);
				couponsParams.setPageNo(0);
			}
			CollectionVO<CouponsTypeVO> payload = new CollectionVO<>(
					new ArrayList<CouponsTypeVO>(), (int) recordCnt,
					couponsParams.getPageSize() < 1 ? (int) recordCnt
							: couponsParams.getPageSize());
			List<CouponsTypeVO> list = couponsTypeService
					.getByCondition(couponsParams);
			payload.addAll(list);
			res = Responses.SUCCESS().setPayload(payload).setCode(1)
					.setMsg("数据获取成功");
		} catch (Exception e) {
			e.printStackTrace();
	...
	public List<MerchantUserEntity> getByStatus(HttpServletRequest request,
			HttpServletResponse response) {
		try {
			Integer status = 0;
			if (request.getParameter("status") != null) {
	...
	public CollectionVO<MerchantUserEntity> getMerchant(
		...
		try {
			...
			if (request.getParameter("realName") != null) {
				query.addCriteria(Criteria.where("realName").is(
			...
			result = new CollectionVO<MerchantUserEntity>(msa, (int) count,
					pageSize);
		} catch (Exception e) {
			logger.error(e.getMessage());
		}
	...
	public CollectionVO<MerchantKitchenInfoEntity> getKitchen(
			@PathVariable Integer pageNo, @PathVariable Integer pageSize,
			HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			query.skip((pageNo - 1) * pageSize);
			query.limit(pageSize);
			List<MerchantKitchenInfoEntity> msa = merchantKitchenService
					.find(query);
			result = new CollectionVO<MerchantKitchenInfoEntity>(msa,
					(int) count, pageSize);
		} catch (Exception e) {
			logger.error(e.getMessage());
		}
	...
	public void checkMerchant(HttpServletRequest request,
			HttpServletResponse response) {
		Integer status = 0;
		if (request.getParameter("id") != null
				&& request.getParameter("status") != null) {
			status = Integer.valueOf(request.getParameter("status"));
			merchantUsersService.checkMerchant(request.getParameter("id"),
					status);
		}
	}
